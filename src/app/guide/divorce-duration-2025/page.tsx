...
function CountBar({ label, count, max, color = 'bg-[#a75b45]', showShare = true }: { label: string; count: number; max: number; color?: string; showShare?: boolean }) {
  return (
    <div className="border-b border-[#f0ece8] py-2.5 last:border-0">
      ...
      <div className="h-2.5 overflow-hidden rounded-full bg-[#f0ece7]"><div className={`h-full rounded-full ${color}`} style={{ width: `${count / max * 100}%` }} /></div>
    </div>
  );
}
  ...
  return (
    <article className="mx-auto max-w-[860px] pb-20 text-[#292520]">
      ...
      <div className="mt-8 grid gap-3 sm:grid-cols-3">
        <div className="rounded-2xl border border-[#e9ded4] bg-white p-5"><span className="text-sm font-semibold text-[#6b625b]">2025년 이혼 신고</span><div className="mt-2 text-3xl font-extrabold tabular-nums text-[#a74126]">88,130<span className="ml-1 text-base">건</span></div></div>
        <div className="rounded-2xl border border-[#e9ded4] bg-white p-5"><span className="text-sm font-semibold text-[#6b625b]">평균 혼인 지속 기간</span><div className="mt-2 text-3xl font-extrabold tabular-nums text-[#605b68]">17.6<span className="ml-1 text-base">년</span></div></div>
      ...
      <section id="ages" className="mt-16 scroll-mt-24">
        ...
        <figure className="rounded-[1.7rem] border border-[#e9ded4] bg-white p-5 sm:p-7"><figcaption className="mb-4 border-b border-[#eee8e1] pb-4"><h3 className="editorial-h3">2025년 이혼 당사자 연령별 건수</h3><p className="editorial-desc mt-1">전국 · 건 · 남편·아내 공통 기준으로 14,235건이 막대 100%</p><div className="mt-3 flex flex-wrap gap-4 text-sm"><span className="flex items-center gap-2"><i className="h-2.5 w-2.5 rounded-full bg-[#a75b45]" />남편</span><span className="flex items-center gap-2"><i className="h-2.5 w-2.5 rounded-full bg-[#605b68]" />아내</span></div></figcaption><div>{divorceAges.map(row => <div key={row.label} className="grid grid-cols-[4.7rem_minmax(0,1fr)] items-center gap-2 border-b border-[#f0ece8] py-2.5 last:border-0 sm:grid-cols-[5.5rem_minmax(0,1fr)]"><span className="text-sm font-semibold text-[#403a35]">{row.label}</span><div className="space-y-1.5"><div className="grid grid-cols-[minmax(0,1fr)_4.5rem] items-center gap-2"><div className="h-2.5 rounded-full bg-[#f0ece7]"><div className="h-full rounded-full bg-[#a75b45]" style={{ width: `${row.husbands / 14235 * 100}%` }} /></div><span className="text-right text-xs tabular-nums text-[#6b625b] sm:text-sm">{format(row.husbands)}</span></div><div className="grid grid-cols-[minmax(0,1fr)_4.5rem] items-center gap-2"><div className="h-2.5 rounded-full bg-[#f0ece7]"><div className="h-full rounded-full bg-[#605b68]" style={{ width: `${row.wives / 14235 * 100}%` }} /></div><span className="text-right text-xs tabular-nums text-[#6b625b] sm:text-sm">{format(row.wives)}</span></div></div></div>)}</div><p className="mt-4 text-xs leading-relaxed text-[#756b63]">15세 미만은 남편·아내 모두 0건이어서 생략했습니다. 연령 미상은 남편 4건, 아내 0건입니다. 자료: <Source href={ageUrl}>KOSIS 「연령(5세)/혼인지속기간(동거기간)별 이혼」</Source></p></figure>
        <div className="mt-5 grid gap-3 sm:grid-cols-2"><div className="rounded-2xl bg-[#fff4ec] p-5"><span className="editorial-desc">남편 평균 이혼 연령</span><strong className="mt-1 block text-2xl tabular-nums text-[#a74126]">51.0세</strong></div><div className="rounded-2xl bg-[#f0eef3] p-5"><span className="editorial-desc">아내 평균 이혼 연령</span><strong className="mt-1 block text-2xl tabular-nums text-[#514c5c]">47.7세</strong></div></div>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="rounded-2xl border border-[#edd5c8] bg-[#fff9f4] p-5"><span className="editorial-desc">남편 연령별 이혼율 최고 · 45~49세</span><strong className="mt-1 block text-2xl tabular-nums text-[#a74126]">인구 1천 명당 7.0건</strong></div><div className="rounded-2xl border border-[#dddbe3] bg-[#f8f7fa] p-5"><span className="editorial-desc">아내 연령별 이혼율 최고 · 40~44세</span><strong className="mt-1 block text-2xl tabular-nums text-[#514c5c]">인구 1천 명당 7.7건</strong></div></div>
