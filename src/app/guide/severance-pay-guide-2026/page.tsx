...
export const metadata: Metadata = {
  ...
  description: '근로기준법에 따른 2026년 법정 퇴직금 계산 공식, 1일 평균임금 산출법, 퇴직소득세 절세 팁(IRP 이전)과 미지급 시 지연이자 대처법을 삼촌생각에서 알기 쉽게 정리해 드립니다.',
  ...
  openGraph: {
    ...
    description: '1일 평균임금 계산식, 2026년 개정 퇴직소득세율, IRP 계좌 이전으로 세금 30% 감면받는 실전 꿀팁을 전해드립니다.',
...
export default function SeverancePayGuidePage() {
  return (
    <article className="max-w-3xl mx-auto py-4">
      ...
      <header className="mb-12 border-b border-zinc-200/80 pb-10">
        <div className="flex items-center gap-2.5 text-xs font-bold text-[#c55232] uppercase tracking-wider mb-4">
          ...
          <span className="text-zinc-400 font-normal">2026.09.22 업데이트</span>
        </div>
        <h1 className="text-3xl sm:text-4xl md:text-[2.6rem] font-extrabold text-[#292520] tracking-tight leading-[1.25] mb-6">
          2026년 퇴직금 계산법 총정리: <br className="hidden sm:inline" />
          평균임금 계산식부터 퇴직세 절세 팁까지
        </h1>
        <p className="text-zinc-600 text-base sm:text-lg leading-relaxed font-medium">
          퇴사를 앞두고 내 통장에 실제로 들어올 퇴직금이 얼마인지 궁금하신가요? 1일 평균임금 계산 방법, 근로기준법상 지급 요건, 그리고 IRP 계좌를 활용해 퇴직소득세를 최대 40%까지 아끼는 핵심 비결을 삼촌생각에서 명쾌하게 풀어드립니다.
        </p>
      </header>

      {/* Quick Summary Callout (AEO Optimized) */}
      <section className="bg-white p-6 sm:p-8 rounded-3xl border border-[#c55232]/30 shadow-xs mb-12">
        <h2 className="text-lg font-bold text-[#292520] mb-3 flex items-center gap-2">
        ...
        <ul className="space-y-2.5 text-sm text-zinc-700 leading-relaxed">
          ...
          <li className="flex items-start gap-2">
            ...
            <span><strong>세금 절세:</strong> 일반 계좌 대신 IRP(개인형 퇴직연금) 계좌로 입금받으면 퇴직소득세 30% 감면 혜택</span>
          </li>
        </ul>
      ...
      {/* Footer CTA Banner */}
      <div className="mt-16 p-8 bg-[#fdfbf7] rounded-3xl border border-zinc-200/80 text-center space-y-4">
        <h3 className="text-xl font-bold text-[#292520]">
          내 퇴직금 실수령액, 지금 바로 확인해보세요
        </h3>
        <p className="text-sm text-zinc-500 max-w-md mx-auto leading-relaxed">
          날짜와 월급만 넣으면 1초 만에 1일 평균임금과 예상 세금을 즉시 계산해 드립니다.
        </p>
        <Link
          href="/severance-pay-calculator"
