...
export const metadata: Metadata = {
  ...
  openGraph: {
    ...
    url: pageUrl,
    images: [{ url: '/images/spending-breakdown-cover-2026.jpg', width: 1536, height: 864, alt: '한국 가정 식탁 위 한식과 영수증' }],
  },
};
...
const history = [
  ...
  { year: 2016, food: 13.9, dining: 13.9, note: '처음으로 격차 0.0%p (동률)' },
  { year: 2019, food: 14.0, dining: 14.6, note: '개편 후 통합조사 — 첫 역전' },
  ...
  { year: 2026, food: 14.8, dining: 16.1, note: '격차 1.3%p로 역대 최대' },
];
...
function SectionTitle({ number, title, subtitle }: { number: string; title: string; subtitle: string }) {
  return (
    <div className="mb-6 rounded-[1.65rem] bg-[#292520] px-6 py-7 text-white sm:px-8">
      ...
      <h2 className="mt-3 break-keep text-balance text-[1.45rem] font-extrabold leading-snug tracking-tight sm:text-[1.85rem]">{title}</h2>
      <p className="mt-3 break-keep text-sm leading-relaxed text-[#e5dcd3] sm:text-base">{subtitle}</p>
    </div>
  );
}
  ...
  return (
    <article className="mx-auto max-w-[860px] pb-20 text-[#292520]">
      ...
      <nav aria-label="위치" className="mb-7 text-sm text-zinc-500">
        ...
        <span className="px-2">/</span>경제·소비 통계
      </nav>
      ...
      <header>
        <div className="relative overflow-hidden rounded-[1.7rem] bg-[#292520] px-6 py-8 text-white sm:px-10 sm:py-11">
          <div className="pointer-events-none absolute bottom-[5.5rem] right-3 top-[6.8rem] hidden w-[45%] overflow-hidden rounded-2xl [mask-image:linear-gradient(to_right,transparent,black_16%)] md:block" aria-hidden="true">
            <Image src="/images/spending-breakdown-cover-2026.jpg" alt="한국 가정 식탁 위 한식과 영수증" fill priority sizes="380px" className="object-cover object-center" />
          </div>
        ...
        <p className="editorial-body mt-7 break-keep text-[#5c544d]">
          한 달에 번 돈에서 가장 큰 몫이 빠져나가는 곳은 어디일까요? 대부분 집세(주거비)나 장보기(식료품)를 먼저 떠올립니다. 그런데 2026년 2분기 가계동향조사 통계표를 직접 열어 보면 1위는 따로 있었습니다.
        </p>
        <p className="editorial-body mt-3 break-keep text-[#5c544d]">
          12개 항목의 순위부터, 그 순위가 지난 14년 동안 두 번 뒤집힌 이유, 소득에 따라 달라지는 우선순위까지 공식 원표로 확인합니다.
        </p>
      </header>
      ...
      <section id="ranking" className="mt-16 scroll-mt-24">
        <SectionTitle number="01" title="12대 항목 중 1위는 무엇이었을까?" subtitle="2026년 2분기 가구당 월평균 소비지출 293만 2천 원을 지출액이 큰 순서로 나열합니다." />
        <div className="editorial-body space-y-4 break-keep">
          <p>
            1위는 <strong>음식·숙박 47만 3천 원(16.1%)</strong>입니다. 이어 <strong>식료품·비주류음료 43만 4천 원(14.8%)</strong>, <strong>주거·수도·광열 35만 7천 원(12.2%)</strong>, <strong>교통·운송 33만 8천 원(11.5%)</strong> 순입니다.
          </p>
          <p>
            식료품(장보기)과 음식·숙박(외식·숙박)은 통계상 서로 다른 항목입니다. 이번 분기에는 외식·숙박비가 장보기 비용보다 <strong>3만 9천 원 더 컸습니다.</strong>
          </p>
        </div>
        <MotionFigure className="mt-6 rounded-[1.7rem] border border-[#e9ded4] bg-white p-5 sm:p-7">
          ...
          <div>
            {items.map((item, index) => (
              <div key={item.label} className="border-b border-[#f0ece8] py-3 last:border-0">
                <div className="mb-1.5 flex items-baseline justify-between gap-3">
                  <span className="text-[15px] font-semibold text-[#403a35]">{index + 1}. {item.label}</span>
                  <span className="shrink-0 text-sm tabular-nums text-[#5d554e]">
                    <strong>{formatWon(item.amount)}</strong>
                    <span className="ml-2 text-xs text-[#827870]">{item.share}%</span>
                    <span className={`ml-2 text-xs font-semibold ${item.yoy > 0 ? 'text-[#a74126]' : 'text-[#365e8a]'}`}>
                      {item.yoy > 0 ? '+' : ''}{item.yoy}%
                    </span>
                  </span>
                </div>
          ...
          <p className="mt-4 text-xs leading-relaxed text-[#756b63]">비중은 소비지출 총액 293만 2천 원을 분모로 계산했습니다. 자료: <Source href={statTableUrl}>국가데이터처 「2026년 2/4분기 가계동향조사 결과 통계표」</Source> 원표 1.1.</p>
        </MotionFigure>
      </section>

      {/* SECTION 2: 식료품 vs 외식 */}
      <section id="twist" className="mt-16 scroll-mt-24">
        <SectionTitle number="02" title="의식주 중에서도, 예상과 다른 1위" subtitle="식료품과 음식·숙박을 나란히 놓고, 전년보다 얼마나 늘었는지도 함께 봅니다." />
        <div className="editorial-body space-y-4 break-keep">
          <p>
            식료품과 음식·숙박을 더하면 <strong>90만 7천 원(30.9%)</strong>으로 전체 소비지출의 3분의 1에 가깝습니다. 두 항목의 증가율도 다릅니다. 식료품은 전년동분기대비 <strong>2.8%</strong> 늘었지만, 음식·숙박은 <strong>3.5%</strong> 늘었습니다.
          </p>
          <p>
            가장 흔한 예상인 <strong>주거·수도·광열(35만 7천 원, 12.2%)</strong>은 3위였습니다. 전기·가스요금과 월세·관리비를 포함하는 항목인데도, 외식·숙박이나 장보기보다는 작았습니다.
          </p>
        </div>
        ...
        {/* 증감률 하이라이트 박스 */}
        <div className="mt-6 rounded-[1.7rem] bg-[#f8f5f0] p-6 border border-[#e8ded4] sm:p-8">
          <h3 className="editorial-h3 text-[#a74126] mb-3">전년 대비 증감률, 가장 많이 늘고 줄어든 항목은?</h3>
          <div className="space-y-3 text-sm text-[#403a35] leading-relaxed">
            <div>
              <strong className="block text-[#292520] mb-1">📈 가장 많이 늘어난 항목</strong>
              <strong>가정용품·가사서비스가 +17.9%</strong>로 12개 항목 중 가장 많이 늘었습니다. 이어 <strong>오락·문화 +7.6%</strong>, <strong>보건 +4.2%</strong> 순입니다.
            </div>
            <div>
              <strong className="block text-[#292520] mb-1">📉 줄어든 항목</strong>
              <strong>교통·운송 -0.5%</strong>과 <strong>주류·담배 -2.2%</strong>는 전년보다 줄었습니다. 국제유가 안정과 음주 문화 변화가 배경입니다.
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="history" className="mt-16 scroll-mt-24">
        <SectionTitle number="03" title="이 순위는 코로나19를 사이에 두고 두 번 뒤집혔다" subtitle="계절 요인을 없애기 위해 2012년부터 2026년까지 같은 2분기만 골라 비교합니다." />
        <div className="editorial-body space-y-4 break-keep">
          <p>
            외식·숙박비가 장보기 비용을 앞선 것은 이번이 처음이 아닙니다. 2019년 통합조사 기준으로 이미 <strong>음식·숙박 14.6%가 식료품 14.0%를 앞섰습니다.</strong>
          </p>
          <p>
            그러나 코로나19로 외식이 급감하고 집밥 수요가 늘면서 2020~2021년에는 식료품 비중이 다시 16%대로 치솟아 역전했습니다. 위드코로나가 시작된 2022년부터 음식·숙박 비중이 다시 식료품을 앞섰고, 격차는 2026년 현재 <strong>1.3%포인트로 가장 크게 벌어져 있습니다.</strong>
          </p>
        </div>
        <MotionFigure className="mt-6 rounded-[1.7rem] border border-[#e9ded4] bg-white p-5 sm:p-7">
          <figcaption className="mb-4 border-b border-[#eee8e1] pb-4">
          ...
          <div className="space-y-4">
            {history.map((row) => (
              <div key={row.year} className={`border-b border-[#f0ece8] pb-3 last:border-0 ${row.note ? 'rounded-xl bg-[#faf7f4] px-3 py-2 -mx-1' : ''}`}>
                <div className="mb-1.5 flex items-baseline justify-between gap-2 text-sm">
                  <strong className={row.note ? 'text-[#292520]' : ''}>{row.year}년 2분기</strong>
                  {row.note && <span className="text-right text-[11px] font-semibold text-[#a74126]">{row.note}</span>}
                </div>
      ...
      {/* SECTION 4: 소득 5분위별 차이 */}
      <section id="quintile" className="mt-16 scroll-mt-24">
        <SectionTitle number="04" title="소득에 따라 우선순위가 다르다" subtitle="소득 하위 20%(1분위)와 상위 20%(5분위)의 지출 구성을 같은 12개 항목으로 비교합니다." />
        <div className="editorial-body space-y-4 break-keep">
          <p>
            소득 하위 20%인 1분위 가구는 <strong>식료품(21.1%)·주거(19.7%)·음식숙박(13.6%)·보건(12.2%)</strong> 네 항목에 소비지출의 <strong>66.5%(91만 3천 원)</strong>를 씁니다.
          </p>
          <p>
            반면 상위 20%인 5분위는 같은 네 항목 비중이 43.9%로 낮고, 대신 <strong>교육(9.4%)</strong>과 <strong>오락·문화(7.9%)</strong>, <strong>교통(13.9%)</strong>에 훨씬 더 많이 씁니다.
          </p>
          <p>
            1분위는 교육비(-21.2%)와 오락·문화비(-8.7%)를 전년보다 줄인 반면, 음식숙박(+15.0%)·보건(+13.5%)·주거비(+6.5%) 같은 <strong>필수 지출은 오히려 늘었습니다.</strong>
          </p>
        </div>
        <MotionFigure className="mt-6 overflow-hidden rounded-[1.7rem] border border-[#e9ded4] bg-white">
          ...
          <div className="divide-y divide-[#f0ece8]">
            {quintile.map((row) => (
              <div key={row.label} className="px-4 py-3.5 sm:px-7">
                <strong className="text-[15px]">{row.label}</strong>
                <div className="mt-1.5 grid grid-cols-[minmax(0,1fr)_8.5rem] items-center gap-2">
                  <Bar value={row.q1Share} max={22} />
                  <span className="text-right text-xs tabular-nums text-[#5d554e]">{formatWon(row.q1)} ({row.q1Share}%)</span>
                </div>
                <div className="mt-1 grid grid-cols-[minmax(0,1fr)_8.5rem] items-center gap-2">
      ...
      <section id="method" className="mt-16 scroll-mt-24">
        <SectionTitle number="05" title="이 숫자는 어떻게 만들어질까?" subtitle="평균과 명목, 소비지출과 가계지출의 차이를 구분합니다." />
        <div className="space-y-4 rounded-[1.7rem] border border-[#e9ded4] bg-white p-6 sm:p-8">
          <div className="rounded-xl border border-[#eee7de] bg-[#f8f5f0] p-4">
            <strong className="block text-[#292520] mb-1 text-sm">1. 조사 방법</strong>
            <p className="text-sm text-[#403a35] leading-relaxed">가계동향조사는 전국 <strong>약 7,200여 표본가구</strong>가 가계부를 기장하는 방식으로 이뤄지며, 매달 표본 일부가 순환 교체되는 연동표본으로 설계돼 있습니다. <Source href={sampleFaqUrl}>국가데이터처 설명</Source></p>
          </div>
          <div className="rounded-xl border border-[#eee7de] bg-[#f8f5f0] p-4">
            <strong className="block text-[#292520] mb-1 text-sm">2. 소비지출 ≠ 가계지출</strong>
            <p className="text-sm text-[#403a35] leading-relaxed"><strong>소비지출</strong>은 이 글에서 다룬 12대 항목의 합입니다. 여기에 세금·사회보험료·이자 등 <strong>비소비지출</strong>을 더한 값이 <strong>가계지출(399만 3천 원)</strong>입니다. 두 용어는 다른 금액을 가리킵니다.</p>
          </div>
          <div className="rounded-xl border border-[#eee7de] bg-[#f8f5f0] p-4">
            <strong className="block text-[#292520] mb-1 text-sm">3. 명목 금액 vs 실질 증가율</strong>
            <p className="text-sm text-[#403a35] leading-relaxed">표의 금액은 물가를 반영하지 않은 <strong>명목 금액</strong>입니다. 소비지출은 전년동분기대비 명목 3.4% 늘었지만, 물가상승분을 뺀 <strong>실질 증가율은 0.4%</strong>에 그쳤습니다. 명목 증가율만 보면 소비가 크게 늘어난 듯 보이지만, 실제 구매력 기준 체감 소비는 거의 제자리걸음이었다는 뜻입니다.</p>
          </div>
          <div className="rounded-xl border border-[#eee7de] bg-[#f8f5f0] p-4">
            <strong className="block text-[#292520] mb-1 text-sm">4. 가구당 평균의 한계</strong>
            <p className="text-sm text-[#403a35] leading-relaxed">모든 금액은 <strong>가구당 평균</strong>입니다. 가구원 수가 다른 가구를 함께 평균한 값이므로 1인당 생활비로 바로 나눌 수 없습니다. 1인 가구만의 지출은 조사 시점과 가구 범위가 달라 이 글의 수치와 직접 합산·비교할 수 없습니다. <Link href="/guide/one-person-household-2025" className="font-semibold text-[#a74126] underline underline-offset-4">1인 가구 지출 읽을거리 →</Link></p>
          </div>
        </div>
      </section>
      ...
      <section id="reading" className="mt-16 scroll-mt-24">
        <SectionTitle number="06" title="한 줄로 정리하면" subtitle="순위·추이·계층 차이를 하나의 문장으로 묶습니다." />

        <div className="grid gap-4 sm:grid-cols-3 mb-6">
          <div className="rounded-2xl border border-[#e8ded4] bg-white p-5">
            <span className="text-xs font-bold text-[#c55232] tracking-wider">POINT 1</span>
            <h4 className="editorial-h3 mt-2 mb-1">1위는 외식·숙박</h4>
            <p className="text-xs text-zinc-600 leading-relaxed">
              장보기도, 집세도 아닌 음식·숙박(47만 3천 원, 16.1%)이 2026년 2분기 소비지출의 가장 큰 항목이었습니다.
            </p>
          </div>
          <div className="rounded-2xl border border-[#e8ded4] bg-white p-5">
            <span className="text-xs font-bold text-[#c55232] tracking-wider">POINT 2</span>
            <h4 className="editorial-h3 mt-2 mb-1">코로나 전후로 두 번 역전</h4>
            <p className="text-xs text-zinc-600 leading-relaxed">
              2019년 첫 역전 → 코로나19로 되돌려짐 → 2022년 이후 다시 역전. 격차는 역대 최대 1.3%p입니다.
            </p>
          </div>
          <div className="rounded-2xl border border-[#e8ded4] bg-white p-5">
            <span className="text-xs font-bold text-[#c55232] tracking-wider">POINT 3</span>
            <h4 className="editorial-h3 mt-2 mb-1">평균과 현실의 차이</h4>
            <p className="text-xs text-zinc-600 leading-relaxed">
              소득 하위 20%는 지출의 ⅔가 식료품·주거·보건에 묶여 있고, 교육비와 문화비를 줄여가고 있습니다.
            </p>
          </div>
        </div>

        <p className="editorial-body break-keep">
          2026년 2분기 가구가 가장 많이 쓴 곳은 장보기가 아니라 <strong>음식·숙박(47만 3천 원, 16.1%)</strong>이었습니다. 이 순위는 2019년 처음 뒤집혔다가 코로나19로 되돌아갔고, 2022년 이후 다시 뒤집혀 격차가 가장 크게 벌어져 있습니다.
        </p>
        <p className="editorial-body mt-3 break-keep">
          다만 이 이야기는 평균의 이야기입니다. 소득 하위 20%에게는 여전히 <strong>장보기와 주거비가 가장 큰 부담</strong>이며, 소비지출의 3분의 2가 식료품·주거·음식숙박·보건 네 항목에 묶여 있습니다.
        </p>
      </section>
